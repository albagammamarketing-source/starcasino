...
# =========================================================
# FILTRI MODIFICATI DALL'APP STREAMLIT
# =========================================================
...
def costruisci_query() -> tuple[str, list[object]]:
    ...
    sport_list = _lista_pulita(DES_SPORT_LIST)
    if sport_list:
        ...
        params.extend([x.upper() for x in sport_list])
        ...
        where_clauses.append(
            ...
            tg.id_ticket IN (
                ...
                HAVING
                    ...
                    AND MAX(TRIM(COALESCE(td_scom.des_scom, '')))
                        IN ({placeholders})
            )
            """
        )
...
def crea_output_ticket(df: pd.DataFrame) -> pd.DataFrame:
    ...
    aggregati = (
        ...
        .agg(
            cf=("cf", "first"),
            num_conto=("num_conto", "first"),
            nome_commerciale=("nome_commerciale", "first"),
            des_stato=("des_stato", "first"),
            data_ora_vend=("data_ora_vend", "first"),
            num_eventi=("num_eventi", "first"),
            Sport=("des_sport", lambda s: ", ".join(sorted(set(x for x in s if x)))),
            **{
                "Codici manifestazione": (
                    "cod_manif",
                ...
                "Importo Giocato": ("importo_pagato", "first"),
                "Importo Vincita Potenziale": (
        ...
        .sort_values(["nome_commerciale", "cf", "id_ticket"])
        .reset_index(drop=True)
    )

    return aggregati[colonne_output]
...
def crea_output_dettaglio(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        dettaglio[colonne_output]
        .sort_values(["id_ticket", "betradar_id"], na_position="last")
        .reset_index(drop=True)
    )
