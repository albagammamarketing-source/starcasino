...
OUTPUT_PAGATO_OP_CSV = BASE_PATH / "3_3moreEvents_pagato_op.csv"
...
def costruisci_query() -> tuple[str, list[object]]:
    ...
    manifestazione_list = [
        x.upper() for x in _lista_pulita(MANIFESTAZIONE_LIST)
    ]
    if manifestazione_list:
        ...
        where_clauses.append(
            ...
            NOT EXISTS (
                ...
                WHERE td_manif.id_ticket = tg.id_ticket
                  AND (
                        TRIM(COALESCE(td_manif.manifestazione, '')) = ''
                        OR UPPER(TRIM(td_manif.manifestazione))
                           NOT IN ({placeholders})
                  )
            )
            """
...
def crea_output_pagato_op(df: pd.DataFrame) -> pd.DataFrame:
    """
    Restituisce una riga per ticket che soddisfa entrambe le condizioni:
    - des_stato del ticket uguale a PAGATO
    - almeno un evento con cod_stato_esito uguale a OP
    """
    ...
    conteggio_op = (
        ...
        .groupby("id_ticket", as_index=False)
        .agg(eventi_OP=("_is_op", "sum"))
    )
    ...
    ticket = (
        ...
        .drop_duplicates(subset=["id_ticket"])
        .merge(conteggio_op, on="id_ticket", how="left", validate="one_to_one")
    )
    ...
    ticket["eventi_OP"] = ticket["eventi_OP"].astype(int)
...
def main() -> None:
    ...
    print(f"Manifestazioni: {MANIFESTAZIONE_LIST or 'TUTTE'}")
    print(
        "Quota minima ticket: "
    ...
    print(f"Ticket con des_stato=PAGATO e almeno un evento OP: {len(pagato_op)}")
